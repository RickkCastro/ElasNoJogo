    ...
    const handleUpload = async (
        ...
        try {
            ...
            const dbResult = await createVideo({
                ...
                thumbnail_url: thumbnailResult.success
                    ? thumbnailResult.url
                    : null,
    ...
    const loadVideos = useCallback(async (page = 0) => {
        ...
        try {
            const result = await getVideos(
                page,
                VIDEO_CONFIG.FEED.ITEMS_PER_PAGE
            );
            if (result.success) {
...
export const useVideoDelete = () => {
    ...
    const deleteVideo = async (videoId, videoUrl, thumbnailUrl) => {
        ...
        setError(null);
        try {
            const result = await deleteVideoFromDB(
                videoId,
                videoUrl,
                thumbnailUrl
            );
...
export const useVideoEdit = () => {
    ...
    const editVideo = async (videoId, { title, description, location }) => {
        setEditing(true);
        setError(null);
        try {
            const result = await updateVideo(videoId, { title, description, location });
            if (!result.success) {
                throw new Error(result.error);
            }
